...
const bad = new RegExp([
	...
	"死[^ぁ-んァ-ン]*に[^ぁ-んァ-ン]*損[^ぁ-んァ-ン]*な[^ぁ-んァ-ン]*い"
].join("|"));
...
exports.run = function(Server, page) {
	...
	Server.post("/buy/:id", function(req, res) {
		if (req.session.profile) {
			...
			MainDB.kkutu_shop.findOne(['_id', gid]).on(function($item) {
				...
				if ($item.cost < 0) return res.json({
					error: 400
				});
	...
	Server.post("/equip/:id", function(req, res) {
		if (!req.session.profile) return res.json({
			error: 400
		});
		var uid = req.session.profile.id;
		var gid = req.params.id;
		...
		MainDB.users.findOne(['_id', uid]).limit(['box', true], ['equip', true]).on(function($user) {
			...
			MainDB.kkutu_shop.findOne(['_id', gid]).limit(['group', true]).on(function($item) {
				...
				if (!Const.AVAIL_EQUIP.includes($item.group)) return res.json({
					error: 400
				});
	...
	Server.post("/searchnick/:nick", function(req, res) {
		if (!req.params.nick) return res.json({
			error: 400
		});
		if (!req.session.profile) return res.json({
			error: 400
	...
	Server.post("/cf", function(req, res) {
		...
		if (tray.length < 1 || tray.length > 6) return res.json({
			error: 400
		});
		MainDB.users.findOne(['_id', uid]).limit(['money', true], ['box', true]).on(function($user) {
			if (!$user) return res.json({
			...
			for (i in tray) {
				...
				if (($user.box[tray[i]] || 0) < req[tray[i]]) return res.json({
					error: 434
				});
			}
			MainDB.kkutu[parseLanguage(word)].findOne(['_id', word]).on(function($dic) {
				...
				if ($user.money < cfr.cost) return res.json({
					error: 407
				});
	...
	Server.get("/dict/:word", function(req, res) {
		...
		if (!DB.findOne) return res.send({
			error: 400
		});
		DB.findOne(['_id', word]).on(function($word) {
			if (!$word) return res.send({
				error: 404
			});
			res.send({
				word: $word._id,
...
function getCFRewards(word, level, blend) {
	...
	if (blend) {
		...
		R.push({
			key: "dictPage",
			value: Math.round(f.len * 0.6),
			rate: 1
		});
		R.push({
			key: "boxB4",
			value: 1,
			rate: Math.min(1, f.lev / 7)
		});
		if (f.lev >= 5) {
			R.push({
				key: "boxB3",
				value: 1,
				rate: Math.min(1, f.lev / 15)
			});
			cost += 10 * f.lev;
			wur += f.lev / 20;
		}
