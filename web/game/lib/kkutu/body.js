...
function checkAge() {
	...
	while (true) {
		...
		if (lv == 4) {
			...
			if (confirm(L['checkAgeCancel'])) return send('caj', {
				answer: "no"
			}, true);
		}
	}
...
function onMessage(data) {
	...
	switch (data.type) {
		...
		case 'friends':
			...
			break;
		case 'friend':
			$data._friends[data.id] = {
				server: (data.stat == "on") ? data.s : false
			};
		...
		case 'friendAdd':
			...
			else akConfirm(i + L['attemptFriendAdd'], function(resp) {
				send('friendAddRes', {
					from: data.from,
					res: resp
				}, true);
		...
		case 'invited':
			...
			else akConfirm(data.from + L['invited'], function(resp) {
				send('inviteRes', {
					from: data.from,
					res: resp
				});
	...
	switch (cmd[0].toLowerCase()) {
		...
		case "/id":
			if (cmd[1]) {
				...
				if (!c) {
					$.post("/searchnick/" + decodeURI(cmd[1]), function(res) {
						if (res.error) notice(L['error_405']);
						else notice("[" + (++c) + "] " + res.id);
					});
				}
			} else {
...
function tryDict(text, callback) {
	...
	if (text.length < 1) return callback({
		error: 404
	});
...
function updateRoomList(refresh) {
	...
	if (len) {
		...
		//else $(".rooms-gaming-original").css('display', $data.opts.ow ? "none" : "");
...
function roomListBar(o) {
	...
	$R = $("<div>").attr('id', "room-" + o.id).addClass("rooms-item")
		// else $R = $("<div>").attr('id', "room-"+o.id).addClass("rooms-item-original")
		...
		.append($("<div>").width(270)
			//if(o.opts.roomadmin) .append($("<div>").addClass("핑ㆍ경험치 2배 / "+opts.join(" / ").toString()))
			//else 
	...
	//if($data.opts.istheme) {
	if (o.opts.onlybeginner) $R.addClass("rooms-beginner" + (o.gaming ? "-gaming" : ""));
	else if (o.rs.broadcaster) $R.addClass("rooms-broadcaster" + (o.gaming ? "-gaming" : ""));
	else if (o.rs.gm) $R.addClass("rooms-admin" + (o.gaming ? "-gaming" : ""));
	else if (o.gaming) $R.addClass("rooms-gaming");
	//}
	else if (o.gaming) $R.addClass("rooms-gaming-original")
...
function renderGoods($target, preId, filter, equip, onClick) {
	...
	if (!equip) equip = {};
	for (i in equip) {
		if (!$data.box.hasOwnProperty(equip[i])) $data.box[equip[i]] = {
			value: 0
		};
	}
	for (i in $data.box) list.push({
		key: i,
		obj: iGoods(i),
		value: $data.box[i]
	});
	...
	for (i in list) {
		...
		if (!isAll)
			if (filter.indexOf(obj.group) == -1) continue;
...
function drawMyGoods(avGroup) {
	...
	renderGoods($("#dress-goods"), 'dress', filter, equip, function(e) {
		...
		if (e.ctrlKey) {
			...
			akConfirm(L['surePayback'] + commify(Math.round((item.cost || 0) * 0.2)) + L['ping'], function(resp) {
				if (!resp) return;
			...
			akConfirm(L['sureConsume'], function(resp) {
				if (!resp) return;
...
function requestEquip(id, isLeft, verify) {
	...
	if (verify) {
		...
		akConfirm(L[already ? 'sureUnequip' : 'sureEquip'] + ": " + L["item_" + id], function(resp) {
			if (resp) {
				$.post("/equip/" + id, {
					isLeft: isLeft
				}, function(res) {
...
function updateCommunity() {
	...
	function requestEditMemo(e) {
		...
		akPrompt(L['friendEditMemo'], function(c) {
			if (!c) return;
			send('friendEdit', {
				id: id,
				memo: c
	...
	if (o.robot) {
		...
		for (i in o.data.record) {
			...
			$rec.append($("<div>").addClass("profile-record-field")
				...
				.append($("<div>").addClass("profile-field-percent").html(String(pct) + '%'))
...
function requestInvite(id) {
	...
	if (id != "AI") {
		...
		akConfirm(nick + L['sureInvite'], function(resp) {
			if (!resp) return;
			send('invite', {
				target: id
			});
...
function onGoods(e) {
	...
	if ($data.box)
		if ($data.box[id]) spt = L['alreadyGot'] + " " + spt;
...
function pushHistory(text, mean, theme, wc) {
	...
	$v.append($w = $("<div>").addClass("history-mean ellipse").append(val))
		.append($x = $("<div>").addClass("expl").css({
				'width': 200,
				'white-space': "normal"
			})
...
function setRoomHead($obj, room) {
	...
	$obj.empty()
		.append(mobile ? '' : '<h5 style="position: absolute;left: 50%;transform: translateX(-50%);font-weight: bold;">KKUTU.XYZ</h5>')
...
function loadSounds(list, callback, isFirstLoad) {
	$data._lsRemain = list.length;

	list.forEach(function(v) {
		getAudio(v.key, v.value, callback, isFirstLoad);
	});
}

function getAudio(k, url, cb, isFirstLoad) {
...
function tryJoin(id) {
	...
	if ($data.rooms[id].password) {
		akPrompt.password(L['putPassword'], function(pass) {
			if (!pass) return;
			$data._pw = pass;
			send('enter', {
				id: id,
...
function chatBalloon(text, id, flag) {
	...
	var $obj = $("<div>").addClass("chat-balloon")
		.attr('id', "cb-" + id)
		.append($("<div>").addClass("jt-image " + img))[(flag == 2) ? 'prepend' : 'append']($("<h4>").text(text));
	var ot, ol;
...
function iImage(key, sObj) {
	...
	} else if (typeof sObj == "string") sObj = {
		_id: "def",
		group: sObj,
		options: {}
	};
