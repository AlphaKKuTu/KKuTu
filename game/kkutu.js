	...
	*/
	...
	my.refresh = function(){
		...
		if(my.guest){
			...
			if(my.nick != "nonick") my.profile.title = my.nick;
			...
			if(black) {
				if(!time) R.go({ result: 444, black: black });
				else R.go({ result: 444, black: black, time: time });
			}
...
exports.Room = function(room, channel){
	...
	my.rsf = function(){
		...
		for(i in my.players) {
			if(!my.players[i] || !DIC[my.players[i]] || !DIC[my.players[i]].equip || !DIC[my.players[i]].equip['BDG']) continue;
			switch(DIC[my.players[i]].equip['BDG']) {
				case "b1_yt":
				case "b1_tw":
				case "b1_bj":
					my.rs.broadcaster = true;
				case "b1_gm":
				case "b1_ad":
				case "b1_user":
				case "b1_word":
	...
	my.RoomSettingOK = function(rw){
		if(my.rs.gm) { rw.score = rw.score * 2; rw.money = rw.money * 2; }
		else if(my.rs.broadcaster) { rw.score = rw.score * 1.5; rw.money = rw.money * 1.5; };
	};
	...
	my.come = function(client){
		...
		if(my.players.push(client.id) == 1) my.master = client.id;
